...
Optimized for Japanese text with proper boundary detection.
"""
...
# Import Japanese processor for enhanced text processing
try:
    from .japanese_processor import get_japanese_processor
except ImportError:
    # Fallback if japanese_processor not available
    get_japanese_processor = None
    logger.warning("Japanese processor not available, using regex fallback")
...
class SemanticChunker:
    ...
    def __init__(self, config: ChunkingConfig):
        ...
        # Initialize Japanese processor for better text handling
        self.jp_processor = None
        if get_japanese_processor is not None:
            try:
                self.jp_processor = get_japanese_processor(use_spacy=True)
                logger.info("Initialized Japanese processor with spaCy")
            except Exception as e:
                logger.warning(f"Failed to initialize Japanese processor: {e}")
    ...
    def _split_on_structure(self, content: str) -> List[str]:
        ...
        Uses spaCy for Japanese text when available, otherwise falls back to regex.
        ...
        Returns:
            List of sections
        """
        # Check if we should use spaCy for Japanese content
        if self.jp_processor and self._has_japanese(content):
            return self._split_japanese_with_spacy(content)
        else:
            return self._split_with_regex(content)

    def _split_japanese_with_spacy(self, content: str) -> List[str]:
        """
        Split Japanese content using spaCy sentence segmentation.
        ...
        Returns:
            List of sections
        """
        # First split on markdown structural elements
        sections = self._split_markdown_structure(content)

        # Then use spaCy to split each section into sentences
        final_sections = []
        for section in sections:
            if self.jp_processor.is_japanese(section):
                # Use spaCy sentence splitting
                try:
                    sentences = self.jp_processor.split_sentences(section)
                    final_sections.extend(sentences)
                except Exception as e:
                    logger.warning(f"spaCy splitting failed, using regex: {e}")
                    # Fallback to regex for this section
    ...
    def _split_with_regex(self, content: str) -> List[str]:
        """
        Original regex-based splitting (fallback method).
        ...
        final_sections = []
        for section in sections:
            if self._has_japanese(section):
                # Split on Japanese sentence enders: 。！？
                jp_sentences = re.split(r'([。！？])', section)
                # Rejoin sentence with its punctuation
                current = ""
                for i, part in enumerate(jp_sentences):
                    current += part
                    if part in '。！？':
    ...
    def _split_markdown_structure(self, content: str) -> List[str]:
        """
        Split content on markdown structural elements.
    ...
    def _simple_split(self, text: str) -> List[str]:
        ...
        while start < len(text):
            ...
            # Try to end at a sentence boundary (English or Japanese)
            chunk_end = end
            for i in range(end, max(start + self.config.min_chunk_size, end - 200), -1):
                # Check for English or Japanese sentence endings
                if text[i] in '.!?\n' or text[i] in '。！？':
                    chunk_end = i + 1
                    break
    ...
    def _create_chunk_objects(
        ...
        for i, chunk_text in enumerate(chunks):
            ...
            # Create chunk metadata with Japanese detection
            has_japanese = self._has_japanese(chunk_text)
            chunk_metadata = {
                **base_metadata,
