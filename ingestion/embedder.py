...
class EmbeddingGenerator:
    ...
    def __init__(
        ...
        # Model-specific configurations (OpenAI + Ollama)
        self.model_configs = {
            # OpenAI models
            "text-embedding-3-small": {"dimensions": 1536, "max_tokens": 8191},
            "text-embedding-3-large": {"dimensions": 3072, "max_tokens": 8191},
            "text-embedding-ada-002": {"dimensions": 1536, "max_tokens": 8191},
            # Ollama models (for Japanese support)
        ...
        # Auto-detect dimensions from environment if available
        ...
        if env_dimensions:
            try:
                ...
                logger.info(f"Using embedding dimensions from env: {dimensions}")
            except ValueError:
                logger.warning(f"Invalid EMBEDDING_DIMENSIONS value: {env_dimensions}")
                self.config = self.model_configs.get(model, {"dimensions": 768, "max_tokens": 8192})
        elif model not in self.model_configs:
            logger.warning(f"Unknown model {model}, using default config (768D)")
